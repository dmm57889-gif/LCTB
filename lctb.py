...
if st.button("🚀 Avvia Elaborazione", type="primary"):
    ...
    try:
        ...
        status_text.text("♻️ Elaborazione articoli riciclati...")
        
        A_raw = A.copy()  # Usiamo i dati già caricati
        A_recycled = A_raw[A_raw.get("Recycled", "") == "Sì"].copy()
        
        if not A_recycled.empty:
            st_item_recycled = st_item.merge(A_recycled, on="Item Code", how='inner')
            df_recycled = st_item_recycled.copy()
            
            for func in df_recycled["Function"].unique():
                st_percentiles = st_item[st_item["Function"] == func]["ST item"].quantile([0.25, 0.5, 0.75])
                
                def categorize(row):
                    if row["ST item"] <= st_percentiles.iloc[0]:
                        return 'Basso'
                    elif row["ST item"] <= st_percentiles.iloc[1]:
                        return 'Medio Basso'
                    elif row["ST item"] <= st_percentiles.iloc[2]:
                        return 'Medio Alto'
                    else:
                        return 'Alto'
                
                mask = df_recycled["Function"] == func
                df_recycled.loc[mask, "ST_Cluster"] = df_recycled[mask].apply(categorize, axis=1)
            
            # Aggiungi colonne mancanti per articoli riciclati
        ...
        if keras_model:
            ...
            try:
                ...
                # Elaborazione immagini (versione semplificata per demo)
                merged_df2["Discount Prediction"] = "Predizione non disponibile"
